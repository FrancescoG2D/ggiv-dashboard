...
def elabora_dati(df):
    ...
    if 'Data_Ultima_News' in df.columns:
        ...
        # FIX: Usa il fuso orario italiano (Europa/Roma) invece dell'UTC del server
        # Questo evita che i "Giorni Silenzio" differiscano di 1 giorno quando l'app
        # gira su Streamlit Cloud (che usa UTC)
...
# FIX: Usa period="2d" e prende il Close del giorno precedente per una
# variazione giornaliera corretta (prima usava Open dello stesso giorno)
...
def get_index_data(ticker):
    try:
        ...
        if len(data) >= 2:
            ...
            change = close_oggi - close_ieri
            change_percent = (change / close_ieri) * 100
            return {
        ...
        # FIX: Era un except silenzioso, ora mostra un avviso invece di fallire
        # in modo invisibile
        st.warning(f"Impossibile recuperare dati per {ticker}: {e}")
        return None
...
# NOTA: Il prezzo e la variazione di GGIV INDEX sono ancora valori fissi di esempio.
# In futuro andrebbero calcolati dinamicamente dai dati del portafoglio.
info_tuo_indice = {
    'ticker': 'GGIV INDEX',
    'price': 10245.50,
    'change_percent': 1.4
...
<style>
    ...
    .ticker-price {{ font-weight: normal !important; }}
...
with tab_overview:
    ...
    # NOTA: Volatilità e Sharpe sono valori di esempio fissi.
    # In futuro andrebbero calcolati dai rendimenti storici del portafoglio.
    ...
    if not df_aziende.empty:
        st.dataframe(df_aziende.style.apply(
            lambda x: ['background-color: #ff4b4b' if i == True else '' for i in (x == x)],
            axis=1,
...
with tab_backtest:
    ...
    # FIX: Aggiunto seed fisso (np.random.seed) per rendere il grafico
    # stabile ad ogni refresh invece di cambiare ogni volta
    np.random.seed(42)
    date = pd.date_range(start="2021-01-01", periods=36, freq="ME")
    df_backtest = pd.DataFrame({
        "Data": date,
        "S&P 500": np.linspace(100, 130, 36) + np.random.normal(0, 2, 36),
        "GGIV Strategy": np.linspace(100, 165, 36) + np.random.normal(0, 3, 36)
    })
    st.plotly_chart(
        px.line(df_backtest, x="Data", y=["S&P 500", "GGIV Strategy"],
    ...
    peso_t1, peso_t2, peso_t3 = [
        df_aziende[df_aziende['Tier'] == t]['Peso_Effettivo'].sum()
        for t in ['Tier 1', 'Tier 2', 'Tier 3']
    ]
    impatto_tot = (
        (-crollo * 1.5 * (peso_t1 / 100)) +
        (-crollo * (peso_t2 / 100)) +
        (-crollo * 0.2 * (peso_t3 / 100))
    )
...
with tab_rischio:
    ...
    if not blocco_scudo and st.button("Calcola Lotti"):
        with st.spinner('Scaricando prezzi...'):
            ...
            # FIX: Funzione separata con @st.cache_data per non ripetere
            # le chiamate HTTP ad ogni click — era la causa principale di lentezza
...
with tab_brevetti:
    ...
    st.metric(
        "Score",
        f"{score}/100",
        "🟢 Eccellente" if score >= 80 else "🟡 Moderato" if score >= 50 else "🔴 Allerta"
    )
